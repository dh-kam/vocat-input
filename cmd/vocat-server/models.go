...
func fetchDynamicGoogleAIStudioModels(ctx context.Context) []ModelOption {
	...
	for _, m := range parsed.Models {
		...
		if strings.Contains(lowerID, "embedding") || strings.Contains(lowerID, "aqa") || strings.Contains(lowerID, "imagen") ||
			...
			strings.Contains(lowerID, "2.0") || strings.Contains(lowerID, "customtools") ||
	...
	// Also include latest Claude models (Sonnet 4.6, Opus 4.6, Fable 4.5, Sonnet 4.5) under Google AI Studio options
	results = append(results,
		...
		ModelOption{ID: "claude-sonnet-4-5", Label: "Claude 4.5 Sonnet", Desc: "High Performance Multimodal"},
	)
